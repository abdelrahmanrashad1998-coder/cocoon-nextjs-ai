...
export function calculateItemPricing(item: QuoteItem): PricedItem {
  ...
  const p = raw.profile ? {
    ...
    accessories_2_leaves: Number(raw.profile.accessories_2_leaves || 0),
    accessories_3_leaves: Number(raw.profile.accessories_3_leaves || 0),
    accessories_4_leaves: Number(raw.profile.accessories_4_leaves || 0),
  ...
  if (raw.type === "curtain_wall") {
    ...
    const frameMeters = Number(raw.frameMeters || raw.designData?.frameMeters || 0);
    const windowMeters = Number(raw.windowMeters || raw.designData?.windowMeters || 0);
    const glassArea = Number(raw.glassArea || raw.designData?.glassArea || 0);

    // panels array -> count windows & doors safely
    const panels = Array.isArray(raw.designData?.panels) ? raw.designData.panels : Array.isArray(raw.panels) ? raw.panels : [];
    const numWindows = panels.filter((pl: any) => String(pl?.type).toLowerCase() === 'window').length;
    const numDoors = panels.filter((pl: any) => String(pl?.type).toLowerCase() === 'door').length;
    const cornerCount = Number(raw.designData?.cornerCount || raw.cornerCount || 0);
    ...
    const framePricePerMeter = Number(raw.designData?.frameProfile?.frame_price
                                ?? raw.designData?.frameProfile?.price
                                ?? p.frame_price
                                ?? 0);

    const windowPricePerMeter = Number(raw.designData?.windowProfile?.frame_price
...
export function calculateQuoteTotals(items: PricedItem[]) {
  ...
  const totalBefore = items.reduce((sum, i) => sum + Number(i.totalBeforeProfit || 0), 0);
  const totalAfter = items.reduce((sum, i) => sum + Number(i.totalPrice || 0), 0);
